...
function getMidiSendOutputTrack(tr)
  ...
  if num_sends>0 then
    ...
    local dest_track=reaper.GetTrack(0,tr_num+bus)
...
function getOriginatingMidiTrack(tr)
  ...
  for i=0,num_receives-1,1 do
    local receive_bus=reaper.BR_GetSetTrackSendInfo(send_track, -1, i, "I_MIDI_SRCBUS", false, 0)
    if receive_bus==bus==0 and 1 or bus then
